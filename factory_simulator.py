...
def main():
    ...
    with st.form("config_form"):
        ...
        with col1:
            form_totalSqft = st.slider("Factory Floor Area (sqft)", 1000, 150000, factory["totalSqft"], step=500)
    ...
    with st.expander("Save / Load Configuration"):
        ...
        # Save: download as JSON with editable filename
        with save_col:
            st.markdown("**Save configuration**")
            if "_save_filename" not in st.session_state:
                st.session_state._save_filename = "factory_config"
            st.text_input("Filename (press Enter to confirm)", key="_save_filename", placeholder="e.g. baseline_v2")
            filename = (st.session_state._save_filename.strip().replace(" ", "_") or "factory_config") + ".json"
            export_data = json.dumps({"version": 2, "factory": factory, "workcells": workcells}, indent=2)
            st.download_button(f"Download {filename}", export_data, filename, "application/json", use_container_width=True)
    ...
    # KPI cards
    tab_results, tab_space, tab_flow, tab_sweep = st.tabs(["Optimization Results", "Space Allocation", "Material Flow", "Factory Size Sweep"])
    ...
    with tab_sweep:
        import plotly.graph_objects as go_sweep
        st.subheader("Factory Size Sweep")
        st.caption("Sweep across factory floor area to see how throughput and required labor scale.")

        s_col1, s_col2, s_col3 = st.columns(3)
        with s_col1:
            sweep_min = st.number_input("Min sqft", value=max(1000, factory["totalSqft"] // 4), step=1000, key="sweep_min")
        with s_col2:
            sweep_max = st.number_input("Max sqft", value=150000, step=1000, key="sweep_max")
        with s_col3:
